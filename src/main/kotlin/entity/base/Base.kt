...
class Base : Node2D(), QueueController, GunController {
	...
	var hp: Control? = null
		set(value) {
			field = value
			hpProgress = value?.findNodeWrapper("Progress")
			hpLabel = value?.findNode("Label")
		}
	...
	private val queueController = QueueControllerImpl(
		...
		creepReady = {
			...
			creepBody.setPosition(entry?.position ?: Vector2.ZERO)
			it.body = creepBody
			it.onDied = { creep ->
	...
	override fun _ready() {
		...
		entry = findNode<Area2D>(R.node.baseEntry)
	...
	override fun _process(delta: Double) {
		...
		hpProgress?.progress = (body?.hp ?: 1).toFloat() / (body?.maxHp ?: 1)
		hpLabel?.text = (body?.hp ?: 0).toString()
	}
